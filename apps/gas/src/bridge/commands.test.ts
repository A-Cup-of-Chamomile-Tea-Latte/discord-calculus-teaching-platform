...
describe("Phase 2B command inbox", () => {
  it("offers fixed actions and metadata-only claims", () => {
    ...
    expect(JSON.stringify(row)).not.toMatch(
      /student|email|message|attachment/i,
    );
    ...
    expect(
      ackLabCommand(
        workbook,
        claim!.envelope.commandId,
        "wrong",
        "APPLIED",
        "2026-08-11T06:02:00Z",
      ),
    ).toBe(false);
    expect(
      ackLabCommand(
  ...
  it("reclaims an expired command and invalidates the stale token", () => {
    ...
    const first = claimLabCommand(
      workbook,
      "a",
      "fp",
      fakeSha,
      "2026-08-11T06:00:01Z",
      "2026-08-11T06:00:02Z",
      "old",
    );
    const second = claimLabCommand(
      workbook,
      "b",
      "fp",
      fakeSha,
      "2026-08-11T06:00:03Z",
      "2026-08-11T06:05:00Z",
      "new",
    );
    ...
    expect(
      ackLabCommand(
        workbook,
        second!.envelope.commandId,
        "old",
        "APPLIED",
        "2026-08-11T06:00:04Z",
      ),
    ).toBe(false);
    expect(
      ackLabCommand(
  ...
  it("emits stale-version and bad-checksum fixtures without new command types", () => {
    ...
    const bad = claimLabCommand(
      workbook,
      "a",
      "fp",
      fakeSha,
      "2026-08-11T06:01:01Z",
      "2026-08-11T06:06:00Z",
      "b",
    );
