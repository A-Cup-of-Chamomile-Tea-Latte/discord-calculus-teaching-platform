...
| **Sheets / Apps Script** | 原型與行政管理層                                                             | 高頻訊息資料庫或逐則 Discord mirror                                   |
...
- `apps/gas/`：Apps Script / Sheets 的本機原型與 mock adapters。
...
- 審查者：[架構概觀](docs/architecture/OVERVIEW.md) → [資料模型](docs/DATA_MODEL_OVERVIEW.md) → [尚未部署](docs/DEPLOYMENT_NOT_DONE.md)。
...
本儲存庫只允許 fixture-first 本機開發。不得提交 secrets、真實學生資料、raw exports 或 deployment IDs；不得在未經明確授權前 push、部署、寄信、建立雲端資源或連線正式 Discord server。部署前置條件與不可由文件自動解除的關卡，見 [尚未部署指南](docs/DEPLOYMENT_NOT_DONE.md)。
