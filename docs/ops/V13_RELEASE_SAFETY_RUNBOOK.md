...
schema 6 或 13／migration names+checksums、fresh health／既有 failure queues、磁碟空間與 inode。只接受已知舊
deployer 或同一 candidate deployer；其他狀態 fail closed。

所有前置條件通過後，它只會 idempotent repair root-owned deployer、建立 consistent backup，並依來源 schema
執行 v6→v13 或 v13→v13 rehearsal receipt；另將既有三個 runtime env 檔案從 `calculus-bot:calculus-bot 0600` 正規化為
