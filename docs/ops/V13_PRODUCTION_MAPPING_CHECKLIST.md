...
套用時先跑 `plan-private-entry`，確認只建立／採用 `開啟隱密案件`、設定該頻道 ACL 與置頂說明，再跑 `ensure-private-entry`。其他 drift 只回報、不修復；既有 Private category 與所有動態案件頻道都必須保留。若 operation log 證明入口是本次新建才可刪除；adopted 入口只能依 before inventory 還原。
